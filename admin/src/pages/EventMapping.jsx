...
//   // Tries, in order:
//   // 1) POST actionPath with { mediaId }
//   // 2) POST actionPath with { id }
//   // 3) POST actionPath with { media: id }
//   // 4) POST actionPath + '/' + id with empty body
...
//         // if server returns 2xx with error-like shape, still proceed — but typical cause is 4xx/5xx
//       }
//     }
...
//   // media mapping wrappers (call doAction with candidate bodies)
...
//           <div style={{ marginBottom: 8 }}>
//             <div style={{ fontWeight: 600, marginBottom: 6 }}>Mapped</div>
//             {Array.isArray(event.media) && event.media.length ? event.media.map(m => (
//               <div key={m._id || `media-mapped-${Math.random()}`} style={itemStyle}>
//                 <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
...
//           <div>
//             <div style={{ fontWeight: 600, marginBottom: 6 }}>Available</div>
//             {Array.isArray(media) && media.filter(m => !mappedMedia.has(String(m._id))).map(m => (
//               <div key={m._id || `media-av-${Math.random()}`} style={itemStyle}>
//                 <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
//                   <MediaThumb item={m} size={72} />
//                   <div>
//                     <div style={{ fontWeight: 700 }}>{m.title || (m.type === 'video' ? 'YouTube video' : 'Image')}</div>
